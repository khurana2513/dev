...
- Name: admin.spec.ts >> @admin fees page shows fee records or empty state
- Location: e2e/admin.spec.ts:88:1
...
Locator: locator('text=Fee Manager').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('text=Fee Manager').first()
...
```ts
  1   | /**
  2   |  * ADMIN FLOWS — requires e2e/.auth/admin.json
  3   |  *
  4   |  * First-time setup (run once, stays valid ~30 days):
  5   |  *   npm run e2e:login:admin
  6   |  *   → Browser opens → click Google Sign-In with your ADMIN account → close browser
  7   |  *
  8   |  * Then run:  npm run e2e:admin
  9   |  */
  10  | 
