...
- Name: admin.spec.ts >> @admin quotation maker — add a student in step 1
- Location: e2e/admin.spec.ts:113:1
...
Locator: locator('button:has-text(\'Add Student\'), button:has-text(\'Add\')').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('button:has-text(\'Add Student\'), button:has-text(\'Add\')').first()
